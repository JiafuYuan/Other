...
(function ($) {
    ...
    */
    function init(target) {
        ...
        var table = $('<table border="0" cellspacing="0" cellpadding="0" style="heigth:100%;"><tr></tr></table>').appendTo(target);
    ...
    function setSize(target) {
        ...
        t.find('>table').height(opts.height);
        t.find('>table').find('div.portal-column').height(opts.height - 20);
        ...
        for (var i = 0; i < columnWidths.length; i++) {
            ...
            var ps = p.find('.panel');
            var h = Math.floor(opts.height / ps.length);
            p.find('div.portal-p').panel('resize', { height: h-20 });
        }
