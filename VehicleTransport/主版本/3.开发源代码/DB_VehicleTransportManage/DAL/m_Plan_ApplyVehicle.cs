...
namespace DB_VehicleTransportManage.DAL
{
	...
	public partial class m_Plan_ApplyVehicle
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle model)
		{
			...
			if (model.i_CheckCount != null)
			{
				strSql1.Append("i_CheckCount,");
				strSql2.Append(""+model.i_CheckCount+",");
			}
			...
			strSql.Append("insert into m_Plan_ApplyVehicle(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Plan_ApplyVehicle set ");
			if (model.ApplyID != null)
			{
			...
			if (model.i_CheckCount != null)
			{
				strSql.Append("i_CheckCount="+model.i_CheckCount+",");
			}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from m_Plan_ApplyVehicle ");
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1  ");
			strSql.Append(" ID,ApplyID,VehicleTypeID,i_Count,i_CheckCount,vc_Memo,i_Flag ");
			strSql.Append(" from m_Plan_ApplyVehicle ");
			strSql.Append(" where ID="+ID+"" );
			DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle model=new DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle();
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle DataRowToModel(DataRow row)
		{
			DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle model=new DB_VehicleTransportManage.Model.m_Plan_ApplyVehicle();
			if (row != null)
			{
				...
				if(row["i_CheckCount"]!=null && row["i_CheckCount"].ToString()!="")
				{
					model.i_CheckCount=int.Parse(row["i_CheckCount"].ToString());
				}
				if(row["vc_Memo"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,ApplyID,VehicleTypeID,i_Count,i_CheckCount,vc_Memo,i_Flag ");
			strSql.Append(" FROM m_Plan_ApplyVehicle ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM m_Plan_ApplyVehicle ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from m_Plan_ApplyVehicle T ");
