...
namespace DB_VehicleTransportManage.DAL
{
	...
	public partial class m_Plan_UnLoad
	{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(DB_VehicleTransportManage.Model.m_Plan_UnLoad model)
		{
			...
			strSql.Append("insert into m_Plan_UnLoad(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_VehicleTransportManage.Model.m_Plan_UnLoad model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Plan_UnLoad set ");
			if (model.PlanID != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from m_Plan_UnLoad ");
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.m_Plan_UnLoad GetModel(int ID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1  ");
			strSql.Append(" ID,PlanID,VehicleID,vc_Memo,i_Flag,AddressID,dt_DateTime,PersonID ");
			strSql.Append(" from m_Plan_UnLoad ");
			strSql.Append(" where ID="+ID+"" );
			DB_VehicleTransportManage.Model.m_Plan_UnLoad model=new DB_VehicleTransportManage.Model.m_Plan_UnLoad();
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.m_Plan_UnLoad DataRowToModel(DataRow row)
		{
			DB_VehicleTransportManage.Model.m_Plan_UnLoad model=new DB_VehicleTransportManage.Model.m_Plan_UnLoad();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,PlanID,VehicleID,vc_Memo,i_Flag,AddressID,dt_DateTime,PersonID ");
			strSql.Append(" FROM m_Plan_UnLoad ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM m_Plan_UnLoad ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from m_Plan_UnLoad T ");
