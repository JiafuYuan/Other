...
namespace DB_VehicleTransportManage.DAL
{
	...
	public partial class m_Rule
	{
		...
		public int Add(DB_VehicleTransportManage.Model.m_Rule model)
		{
			...
			strSql.Append("insert into m_Rule(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update m_Rule set ");
			if (model.vc_Name != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from m_Rule ");
		...
		public DB_VehicleTransportManage.Model.m_Rule GetModel(int ID)
		{
			...
			strSql.Append(" from m_Rule ");
			strSql.Append(" where ID="+ID+"" );
			DB_VehicleTransportManage.Model.m_Rule model=new DB_VehicleTransportManage.Model.m_Rule();
		...
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.m_Rule DataRowToModel(DataRow row)
		{
			DB_VehicleTransportManage.Model.m_Rule model=new DB_VehicleTransportManage.Model.m_Rule();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,vc_Name,vc_Memo,i_Flag ");
			strSql.Append(" FROM m_Rule ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM m_Rule ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from m_Rule T ");
