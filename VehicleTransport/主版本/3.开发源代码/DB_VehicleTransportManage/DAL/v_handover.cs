/**  版本信息模板在安装目录下，可自行修改。
* v_handover.cs
*
* 功 能： N/A
* 类 名： v_handover
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2014/10/30 10:59:18   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/
using System;
using System.Data;
using System.Text;
using System.Data.SqlClient;

namespace DB_VehicleTransportManage.DAL
{
	/// <summary>
	/// 数据访问类:v_handover
	/// </summary>
	public partial class v_handover
	{
		public v_handover()
		{}
		#region  Method



		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(DB_VehicleTransportManage.Model.v_handover model)
		{
			StringBuilder strSql=new StringBuilder();
			StringBuilder strSql1=new StringBuilder();
			StringBuilder strSql2=new StringBuilder();
			if (model.PlanID != null)
			{
				strSql1.Append("PlanID,");
				strSql2.Append(""+model.PlanID+",");
			}
			if (model.VehicleID != null)
			{
				strSql1.Append("VehicleID,");
				strSql2.Append(""+model.VehicleID+",");
			}
			if (model.carcode != null)
			{
				strSql1.Append("carcode,");
				strSql2.Append("'"+model.carcode+"',");
			}
			if (model.ReceiveVehiclePersonID != null)
			{
				strSql1.Append("ReceiveVehiclePersonID,");
				strSql2.Append(""+model.ReceiveVehiclePersonID+",");
			}
			if (model.personname != null)
			{
				strSql1.Append("personname,");
				strSql2.Append("'"+model.personname+"',");
			}
			if (model.dt_HandoverDateTime != null)
			{
				strSql1.Append("dt_HandoverDateTime,");
				strSql2.Append("'"+model.dt_HandoverDateTime+"',");
			}
			if (model.MaterieTypeID != null)
			{
				strSql1.Append("MaterieTypeID,");
				strSql2.Append(""+model.MaterieTypeID+",");
			}
			if (model.matername != null)
			{
				strSql1.Append("matername,");
				strSql2.Append("'"+model.matername+"',");
			}
			if (model.i_HandoverCount != null)
			{
				strSql1.Append("i_HandoverCount,");
				strSql2.Append(""+model.i_HandoverCount+",");
			}
			if (model.n_Count != null)
			{
				strSql1.Append("n_Count,");
				strSql2.Append(""+model.n_Count+",");
			}
			if (model.AddressID != null)
			{
				strSql1.Append("AddressID,");
				strSql2.Append(""+model.AddressID+",");
			}
			if (model.addressname != null)
			{
				strSql1.Append("addressname,");
				strSql2.Append("'"+model.addressname+"',");
			}
			strSql.Append("insert into v_handover(");
			strSql.Append(strSql1.ToString().Remove(strSql1.Length - 1));
			strSql.Append(")");
			strSql.Append(" values (");
			strSql.Append(strSql2.ToString().Remove(strSql2.Length - 1));
			strSql.Append(")");
            int rows = DB.OleDbHelper.ExecuteSql(strSql.ToString());
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(DB_VehicleTransportManage.Model.v_handover model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update v_handover set ");
			if (model.PlanID != null)
			{
				strSql.Append("PlanID="+model.PlanID+",");
			}
			else
			{
				strSql.Append("PlanID= null ,");
			}
			if (model.VehicleID != null)
			{
				strSql.Append("VehicleID="+model.VehicleID+",");
			}
			else
			{
				strSql.Append("VehicleID= null ,");
			}
			if (model.carcode != null)
			{
				strSql.Append("carcode='"+model.carcode+"',");
			}
			else
			{
				strSql.Append("carcode= null ,");
			}
			if (model.ReceiveVehiclePersonID != null)
			{
				strSql.Append("ReceiveVehiclePersonID="+model.ReceiveVehiclePersonID+",");
			}
			else
			{
				strSql.Append("ReceiveVehiclePersonID= null ,");
			}
			if (model.personname != null)
			{
				strSql.Append("personname='"+model.personname+"',");
			}
			else
			{
				strSql.Append("personname= null ,");
			}
			if (model.dt_HandoverDateTime != null)
			{
				strSql.Append("dt_HandoverDateTime='"+model.dt_HandoverDateTime+"',");
			}
			else
			{
				strSql.Append("dt_HandoverDateTime= null ,");
			}
			if (model.MaterieTypeID != null)
			{
				strSql.Append("MaterieTypeID="+model.MaterieTypeID+",");
			}
			else
			{
				strSql.Append("MaterieTypeID= null ,");
			}
			if (model.matername != null)
			{
				strSql.Append("matername='"+model.matername+"',");
			}
			else
			{
				strSql.Append("matername= null ,");
			}
			if (model.i_HandoverCount != null)
			{
				strSql.Append("i_HandoverCount="+model.i_HandoverCount+",");
			}
			else
			{
				strSql.Append("i_HandoverCount= null ,");
			}
			if (model.n_Count != null)
			{
				strSql.Append("n_Count="+model.n_Count+",");
			}
			else
			{
				strSql.Append("n_Count= null ,");
			}
			if (model.AddressID != null)
			{
				strSql.Append("AddressID="+model.AddressID+",");
			}
			else
			{
				strSql.Append("AddressID= null ,");
			}
			if (model.addressname != null)
			{
				strSql.Append("addressname='"+model.addressname+"',");
			}
			else
			{
				strSql.Append("addressname= null ,");
			}
			int n = strSql.ToString().LastIndexOf(",");
			strSql.Remove(n, 1);
			strSql.Append(" where ");
            int rowsAffected = DB.OleDbHelper.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete()
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from v_handover ");
			strSql.Append(" where " );
            int rowsAffected = DB.OleDbHelper.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.v_handover GetModel()
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1  ");
			strSql.Append(" PlanID,VehicleID,carcode,ReceiveVehiclePersonID,personname,dt_HandoverDateTime,MaterieTypeID,matername,i_HandoverCount,n_Count,AddressID,addressname ");
			strSql.Append(" from v_handover ");
			strSql.Append(" where " );
			DB_VehicleTransportManage.Model.v_handover model=new DB_VehicleTransportManage.Model.v_handover();
            DataSet ds = DB.OleDbHelper.GetDataSet(strSql.ToString());
			if(ds.Tables[0].Rows.Count>0)
			{
				return DataRowToModel(ds.Tables[0].Rows[0]);
			}
			else
			{
				return null;
			}
		}

		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public DB_VehicleTransportManage.Model.v_handover DataRowToModel(DataRow row)
		{
			DB_VehicleTransportManage.Model.v_handover model=new DB_VehicleTransportManage.Model.v_handover();
			if (row != null)
			{
				if(row["PlanID"]!=null && row["PlanID"].ToString()!="")
				{
					model.PlanID=int.Parse(row["PlanID"].ToString());
				}
				if(row["VehicleID"]!=null && row["VehicleID"].ToString()!="")
				{
					model.VehicleID=int.Parse(row["VehicleID"].ToString());
				}
				if(row["carcode"]!=null)
				{
					model.carcode=row["carcode"].ToString();
				}
				if(row["ReceiveVehiclePersonID"]!=null && row["ReceiveVehiclePersonID"].ToString()!="")
				{
					model.ReceiveVehiclePersonID=int.Parse(row["ReceiveVehiclePersonID"].ToString());
				}
				if(row["personname"]!=null)
				{
					model.personname=row["personname"].ToString();
				}
				if(row["dt_HandoverDateTime"]!=null && row["dt_HandoverDateTime"].ToString()!="")
				{
					model.dt_HandoverDateTime=DateTime.Parse(row["dt_HandoverDateTime"].ToString());
				}
				if(row["MaterieTypeID"]!=null && row["MaterieTypeID"].ToString()!="")
				{
					model.MaterieTypeID=int.Parse(row["MaterieTypeID"].ToString());
				}
				if(row["matername"]!=null)
				{
					model.matername=row["matername"].ToString();
				}
				if(row["i_HandoverCount"]!=null && row["i_HandoverCount"].ToString()!="")
				{
					model.i_HandoverCount=int.Parse(row["i_HandoverCount"].ToString());
				}
				if(row["n_Count"]!=null && row["n_Count"].ToString()!="")
				{
					model.n_Count=decimal.Parse(row["n_Count"].ToString());
				}
				if(row["AddressID"]!=null && row["AddressID"].ToString()!="")
				{
					model.AddressID=int.Parse(row["AddressID"].ToString());
				}
				if(row["addressname"]!=null)
				{
					model.addressname=row["addressname"].ToString();
				}
			}
			return model;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select PlanID,VehicleID,carcode,ReceiveVehiclePersonID,personname,dt_HandoverDateTime,MaterieTypeID,matername,i_HandoverCount,n_Count,AddressID,addressname ");
			strSql.Append(" FROM v_handover ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			return DB.OleDbHelper.GetDataSet(strSql.ToString());
		}

		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
			{
				strSql.Append(" top "+Top.ToString());
			}
			strSql.Append(" PlanID,VehicleID,carcode,ReceiveVehiclePersonID,personname,dt_HandoverDateTime,MaterieTypeID,matername,i_HandoverCount,n_Count,AddressID,addressname ");
			strSql.Append(" FROM v_handover ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			strSql.Append(" order by " + filedOrder);
			return DB.OleDbHelper.GetDataSet(strSql.ToString());
		}

		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM v_handover ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
            object obj = DB.OleDbHelper.ExecuteSql(strSql.ToString());
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("SELECT * FROM ( ");
			strSql.Append(" SELECT ROW_NUMBER() OVER (");
			if (!string.IsNullOrEmpty(orderby.Trim()))
			{
				strSql.Append("order by T." + orderby );
			}
			else
			{
				strSql.Append("order by T. desc");
			}
			strSql.Append(")AS Row, T.*  from v_handover T ");
			if (!string.IsNullOrEmpty(strWhere.Trim()))
			{
				strSql.Append(" WHERE " + strWhere);
			}
			strSql.Append(" ) TT");
			strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
			return DB.OleDbHelper.GetDataSet(strSql.ToString());
		}

		/*
		*/

		#endregion  Method
		#region  MethodEx

		#endregion  MethodEx
	}
}

