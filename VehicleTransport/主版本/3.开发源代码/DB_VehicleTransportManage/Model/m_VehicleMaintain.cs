/**  版本信息模板在安装目录下，可自行修改。
* m_VehicleMaintain.cs
*
* 功 能： N/A
* 类 名： m_VehicleMaintain
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2014-08-11 11:08:37   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/
using System;
namespace DB_VehicleTransportManage.Model
{
	/// <summary>
	/// m_VehicleMaintain:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class m_VehicleMaintain
	{
		public m_VehicleMaintain()
		{}
		#region Model
		private int _id;
		private int? _vehicleid;
		private DateTime? _dt_begindatetime;
		private string _dt_enddatetime;
		private int? _recordpersonid;
		private string _vc_personname;
		private string _vc_content;
		private string _vc_memo;
		private int? _i_flag=0;
		/// <summary>
		/// 
		/// </summary>
		public int ID
		{
			set{ _id=value;}
			get{return _id;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? VehicleID
		{
			set{ _vehicleid=value;}
			get{return _vehicleid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? dt_BeginDateTime
		{
			set{ _dt_begindatetime=value;}
			get{return _dt_begindatetime;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string dt_EndDateTime
		{
			set{ _dt_enddatetime=value;}
			get{return _dt_enddatetime;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? RecordPersonID
		{
			set{ _recordpersonid=value;}
			get{return _recordpersonid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string vc_PersonName
		{
			set{ _vc_personname=value;}
			get{return _vc_personname;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string vc_Content
		{
			set{ _vc_content=value;}
			get{return _vc_content;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string vc_Memo
		{
			set{ _vc_memo=value;}
			get{return _vc_memo;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? i_Flag
		{
			set{ _i_flag=value;}
			get{return _i_flag;}
		}
		#endregion Model

	}
}

