package com.bestway.kj915.activity.home.unloadvehicle;
...
public class UnLoadVehicleActivity extends BasicTitleActivity {
		...
		new AlertDialog.Builder(this).setTitle("是否确定提交")
				...
				.setPositiveButton("确定", new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						ReqUnLoad unLoad = new ReqUnLoad();
						unLoad.DateTime = TimerUtils.getTime();
						unLoad.UserID = GlobleFields.UserID;
						unLoad.AddressID = GlobleFields.getHashMap_Address(
								UnLoadVehicleActivity.this).get(
								destination.getCurrentText()).ID;

						// 准备数据
						List<m_Plan_UnLoad> list = new ArrayList<m_Plan_UnLoad>();

						m_Plan_UnLoad plan_UnLoad = new m_Plan_UnLoad();
						plan_UnLoad.dt_DateTime = TimerUtils.getTime();
						plan_UnLoad.MaterieTypeID = map_MaterielType
								.get(catagory.getCurrentText()).ID;
						plan_UnLoad.n_Count = Float.parseFloat(count.getText()
								.toString());
						plan_UnLoad.AddressID = GlobleFields
								.getHashMap_Address(UnLoadVehicleActivity.this)
								.get(destination.getCurrentText()).ID;
						plan_UnLoad.VehicleID = GlobleFields.getHashMap_Card(
								UnLoadVehicleActivity.this).get(number);
						list.add(plan_UnLoad);

						unLoad.ListM_Plan_UnLoad = new UnloadContainer(list);

						final FinalNClient client = FinalNClient.getInstance();

						client.sendMessage(unLoad, new NetCallback() {
