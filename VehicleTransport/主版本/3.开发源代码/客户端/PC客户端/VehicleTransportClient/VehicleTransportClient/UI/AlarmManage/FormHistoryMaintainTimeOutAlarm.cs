...
{
    public partial class FormHistoryMaintainTimeOutAlarm : Form
    {
        private EnumAlarmType _type;
        private string _tableName = "FormHistoryMaintainTimeOutAlarm";
        private int _selectIndex = -1;
        public FormHistoryMaintainTimeOutAlarm(EnumAlarmType type)
        {
            InitializeComponent();
            _type = type;
            ...
            Pub.StyleManager.SetGridStyle(_tableName, this.dgvList);
            this.Load += new EventHandler(FormHistoryMaintainTimeOutAlarm_Load);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void FormHistoryMaintainTimeOutAlarm_Load(object sender, EventArgs e)
        {
        ...
        public void LoadHistoryAlarmData(string dtStart, string dtStop)
        {
            ...
            {
                int i = dgvList.Rows.Add(item.vc_Code,item.vc_Name,item.dt_Start == null ? "" : item.dt_Start.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"), item.dt_End == null ? "" : item.dt_End.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"));
