...
{
    public partial class FormHistoryGiveTimeOutAlarm : Form
    {
        private EnumAlarmType _type;
        private string _tableName = "FormHistoryGiveTimeOutAlarm";
        private int _selectIndex = -1;
        private string strAlarmType = "";
        public FormHistoryGiveTimeOutAlarm(EnumAlarmType type)
        {
            InitializeComponent();
            _type = type;
            ...
            Pub.StyleManager.SetGridStyle(_tableName, this.dgvList);
            this.Load += new EventHandler(FormHistoryGiveTimeOutAlarm_Load);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void FormHistoryGiveTimeOutAlarm_Load(object sender, EventArgs e)
        {
        ...
        public void LoadHistoryAlarmData(string dtStart, string dtStop)
        {
            ...
            foreach (DB_VehicleTransportManage.Model.v_VehicleAlarm item in lst)
            {
                int i = dgvList.Rows.Add(item.vc_PlanCode,item.ApplyDepartmentName, item.ArriveDestinationAddressName,
                    item.PlanGiveCarDepartmentName,item.dt_Start == null ? "" : item.dt_Start.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"), item.dt_End == null ? "" : item.dt_End.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"));
