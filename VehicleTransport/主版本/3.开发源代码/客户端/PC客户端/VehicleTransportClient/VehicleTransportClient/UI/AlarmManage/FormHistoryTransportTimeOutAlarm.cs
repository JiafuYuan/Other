...
{
    public partial class FormHistoryTransportTimeOutAlarm : Form
    {
        private EnumAlarmType _type;
        private string _tableName = "FormHistoryTransportTimeOutAlarm";
        private int _selectIndex = -1;
        public FormHistoryTransportTimeOutAlarm(EnumAlarmType type)
        {
            InitializeComponent();
            _type = type;
            ...
            Pub.StyleManager.SetGridStyle(_tableName, this.dgvList);
            this.Load += new EventHandler(FormHistoryTransportTimeOutAlarm_Load);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void FormHistoryTransportTimeOutAlarm_Load(object sender, EventArgs e)
        {
        ...
        public void LoadHistoryAlarmData(string dtStart, string dtStop)
        {
            ...
            foreach (DB_VehicleTransportManage.Model.v_VehicleAlarm item in lst)
            {
                int i = dgvList.Rows.Add(item.vc_PlanCode,item.ApplyDepartmentName, item.ArriveDestinationAddressName,
                    item.dt_ArriveDestinationDateTime,item.dt_Start == null ? "" : item.dt_Start.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"), item.dt_End == null ? "" : item.dt_End.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"));
