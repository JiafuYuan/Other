...
{
    public partial class FormHistoryLoadTimeOutAlarm : Form
    {
        private EnumAlarmType _type;
        private string _tableName = "FormHistoryLoadTimeOutAlarm";
        private int _selectIndex = -1;
        public FormHistoryLoadTimeOutAlarm(EnumAlarmType type)
        {
            InitializeComponent();
            _type = type;
            ...
            Pub.StyleManager.SetGridStyle(_tableName, this.dgvList);
            this.Load += new EventHandler(FormHistoryLoadTimeOutAlarm_Load);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void FormHistoryLoadTimeOutAlarm_Load(object sender, EventArgs e)
        {
        ...
        public void LoadHistoryAlarmData(string dtStart, string dtStop)
        {
            ...
            foreach (DB_VehicleTransportManage.Model.v_VehicleAlarm item in lst)
            {
                int i = dgvList.Rows.Add(item.vc_PlanCode,item.ApplyDepartmentName, item.ArriveDestinationAddressName,
                    item.PlanLoadDepartmentName,item.dt_Start == null ? "" : item.dt_Start.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"), item.dt_End == null ? "" : item.dt_End.Value.ToString("yyyy年MM月dd日 HH时mm分ss秒"));
