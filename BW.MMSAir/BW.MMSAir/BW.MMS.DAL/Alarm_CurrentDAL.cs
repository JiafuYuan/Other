...
{
    /// <summary>
    /// 数据访问类:Alarm_CurrentDAL
    /// </summary>
    public partial class Alarm_CurrentDAL
    {
        public Alarm_CurrentDAL()
        { }
        #region  Method
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "Alarm_Current");
        }
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from Alarm_Current");
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(BW.MMS.Model.Alarm_CurrentEntity model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Alarm_Current(");
            strSql.Append("ID,AlarmTypeID,AreaID,vc_Address,dt_BeginTime,vc_AlarmReason,vc_UnAlarmPlan,DeptID,vc_DeptName,PersonID,vc_PersonNmae,vc_Memo,i_Flag)");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(BW.MMS.Model.Alarm_CurrentEntity model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Alarm_Current set ");
            ...
            parameters[4].Value = model.vc_AlarmReason;
            parameters[5].Value = model.vc_UnAlarmPlan;
            parameters[6].Value = model.DeptID;
            parameters[7].Value = model.vc_DeptName;
            parameters[8].Value = model.PersonID;
            parameters[9].Value = model.vc_PersonNmae;
            parameters[10].Value = model.vc_Memo;
            parameters[11].Value = model.i_Flag;
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Alarm_Current ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)			};
            parameters[0].Value = ID;
        ...
        /// 得到一个对象实体
        /// </summary>
        public BW.MMS.Model.Alarm_CurrentEntity GetModel(int ID)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,AlarmTypeID,AreaID,vc_Address,dt_BeginTime,vc_AlarmReason,vc_UnAlarmPlan,DeptID,vc_DeptName,PersonID,vc_PersonNmae,vc_Memo,i_Flag from Alarm_Current ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)			};
            parameters[0].Value = ID;

            BW.MMS.Model.Alarm_CurrentEntity model = new BW.MMS.Model.Alarm_CurrentEntity();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,AlarmTypeID,AreaID,vc_Address,dt_BeginTime,vc_AlarmReason,vc_UnAlarmPlan,DeptID,vc_DeptName,PersonID,vc_PersonNmae,vc_Memo,i_Flag ");
            strSql.Append(" FROM Alarm_Current ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from Alarm_Current T ");
        ...
        /// 分页数据
        /// </summary>
        /// <param name="alarmTypeID">告警类型编号</param>
        ...
        /// <returns></returns>
        public DataTable GetPagingList(string alarmTypeID, int page, int rows, string sort, string order, out int total)
        {
            ...
            param.PageSize = rows;
            param.ReturnFields = " [ID],[vc_Code],[DeptName],[AreaName],[vc_Address],[vc_AlarmName],[dt_BeginTime],[CumulativeDate],[vc_AlarmReason] ";
            param.TableName = " v_Alarm_Current ";
            string strWhere = " where 1=1 ";
            if (!string.IsNullOrEmpty(alarmTypeID))
            {
                strWhere += string.Format(" and AlarmTypeID={0} ", alarmTypeID);
            }
        ...
        public DataSet GetList(int PageSize,int PageIndex,string strWhere)
        {
            ...
            parameters[0].Value = "Alarm_Current";
