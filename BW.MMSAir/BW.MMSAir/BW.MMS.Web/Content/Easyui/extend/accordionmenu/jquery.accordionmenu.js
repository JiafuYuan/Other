...
(function ($) {
    $.fn.accordionmenu = function (options, param) {
        ...
        var accordion = this;
        $(accordion).accordion({ animate: true, fit: true, border: false });
        ...
        if (options.data && options.data.length > 0) {
            $.each(options.data, function (i, n) {
                ...
                $.each(n.children, function (j, o) {
                    var menuitem = $('<li/>');
                    var d = $('<div/>').css('white-space', 'nowrap');
                    var a = $('<a/>').attr('id', o.id).attr('rel', o.attributes.url);
                    var icon = $('<span/>').css('background-position-x', 'left').addClass("accordionmenu-item").addClass(o.iconCls).text(o.text);
                    icon.appendTo(a);
                    a.appendTo(d);
                    d.appendTo(menuitem);
        ...
        $(accordion).accordion({ animate: $(accordion).accordion('panels').length > 2 });
