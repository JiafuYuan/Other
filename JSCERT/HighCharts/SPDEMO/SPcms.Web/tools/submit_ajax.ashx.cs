...
namespace SPcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void demo(HttpContext context)
        {
            ...
            string selid = SPRequest.GetFormString("selid");
            string str = "";
            str = "[45.0,26.8,12.8]";
            //if (selid == "1")
            //{

            //    str = "['恶意程序',45.0],['网页篡改',26.8],['网页挂马',12.8],['漏洞',8.5],['网页仿冒',6.2],['其它',0.7]";
            //}
            //if (selid == "2")
            //{
            //    str = "['恶意程序',15.0],['网页篡改',26.8],['网页挂马',12.8],['漏洞',8.5],['网页仿冒',6.2],['其它',30.7]";
            //}
            //if (selid == "3")
            //{
            //    str = "['恶意程序',11.0],['网页篡改',26.8],['网页挂马',12.8],['漏洞',8.5],['网页仿冒',10.2],['其它',30.7]";
            //}
            //if (selid == "4")
            //{
            //    str = "['恶意程序',15.0],['网页篡改',16.8],['网页挂马',12.8],['漏洞',8.5],['网页仿冒',16.2],['其它',30.7]";
            //}
          



            //DataTable dt=new DataTable("dataCharts");
            //dt.Columns.Add("name");
        ...
        private void comment_add(HttpContext context)
        {
            ...
            string code = SPRequest.GetFormString("txtCode");
            int article_id = SPRequest.GetQueryInt("article_id");
        ...
        {
            int article_id = SPRequest.GetQueryInt("article_id");
            int page_index = SPRequest.GetQueryInt("page_index");
            int page_size = SPRequest.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
        ...
        {
            string username = SPRequest.GetString("param");
            //如果为Null，退出
        ...
        {
            string code = SPRequest.GetFormString("txtCode").Trim();
            string invitecode = SPRequest.GetFormString("txtInviteCode").Trim();
            string username = Utils.ToHtml(SPRequest.GetFormString("txtUserName").Trim());
            string password = SPRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(SPRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(SPRequest.GetFormString("txtMobile").Trim());
            string userip = SPRequest.GetIP();
            ...
            {
                new BLL.user_amount_log().Add(model.id, model.user_name, SPEnums.AmountTypeEnum.SysGive.ToString(), modelGroup.amount, "注册赠送金额", 1);
            }
            ...
            else if (userConfig.regmsgstatus == 2) //发送邮件
            {
                ...
                if (mailModel != null)
                {
                    ...
                    //发送邮件
                    SPMail.sendMail(siteConfig.emailsmtp, siteConfig.emailusername, siteConfig.emailpassword, siteConfig.emailnickname,
            ...
            {
                context.Session[SPKeys.SESSION_USER_INFO] = model;
                context.Session.Timeout = 45;

                //防止Session提前过期
                Utils.WriteCookie(SPKeys.COOKIE_USER_NAME_REMEMBER, "SPcms", model.user_name);
                Utils.WriteCookie(SPKeys.COOKIE_USER_PWD_REMEMBER, "SPcms", model.password);
        ...
        private string verify_email(Model.users userModel)
        {
            ...
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, SPEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = SPEnums.CodeEnum.RegVerify.ToString();
            ...
            try
            {
                SPMail.sendMail(siteConfig.emailsmtp,
        ...
        private string verify_mobile(Model.users userModel)
        {
            ...
            Model.user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, SPEnums.CodeEnum.RegVerify.ToString(), "n");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + SPEnums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + SPEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            ...
            codeModel.type = SPEnums.CodeEnum.Register.ToString();
        ...
        {
            string mobile = SPRequest.GetFormString("mobile");
            if (mobile == "")
            {
            ...
            //写入SESSION，保存验证码
            context.Session[SPKeys.SESSION_SMS_CODE] = strcode;
        ...
        {
            string username = SPRequest.GetFormString("username");
            //检查是否过快
        ...
        {
            string username = SPRequest.GetFormString("txtUserName");
            string password = SPRequest.GetFormString("txtPassword");
            string remember = SPRequest.GetFormString("chkRemember");
            //检查用户名密码
            ...
            {
                Utils.WriteCookie(SPKeys.COOKIE_USER_NAME_REMEMBER, "SPcms", model.user_name, 43200);
                Utils.WriteCookie(SPKeys.COOKIE_USER_PWD_REMEMBER, "SPcms", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = SPRequest.GetString("txtUserName");
            string password = SPRequest.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string email = Utils.ToHtml(SPRequest.GetFormString("txtEmail"));
            string nick_name = Utils.ToHtml(SPRequest.GetFormString("txtNickName"));
            string sex = SPRequest.GetFormString("rblSex");
            string birthday = SPRequest.GetFormString("txtBirthday");
            string telphone = Utils.ToHtml(SPRequest.GetFormString("txtTelphone"));
            string mobile = Utils.ToHtml(SPRequest.GetFormString("txtMobile"));
            string qq = Utils.ToHtml(SPRequest.GetFormString("txtQQ"));
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = SPRequest.GetFormString("hideFileName");
            int x1 = SPRequest.GetFormInt("hideX1");
            int y1 = SPRequest.GetFormInt("hideY1");
            int w = SPRequest.GetFormInt("hideWidth");
            int h = SPRequest.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = SPRequest.GetFormString("txtOldPassword");
            string password = SPRequest.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        {
            string code = SPRequest.GetFormString("txtCode");
            string username = SPRequest.GetFormString("txtUserName").Trim();
            //检查用户名是否正确
            if (string.IsNullOrEmpty(username))
            {
            ...
            BLL.user_code codeBll = new BLL.user_code();
            Model.user_code codeModel;
            codeModel = codeBll.GetModel(username, SPEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                //写入数据库
                codeModel.user_id = model.id;
                codeModel.user_name = model.user_name;
                codeModel.type = SPEnums.CodeEnum.Password.ToString();
                codeModel.str_code = strcode;
        ...
        private void user_message_delete(HttpContext context)
        {
            ...
            string check_id = SPRequest.GetFormString("checkId");
            if (check_id == "")
            {
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string code = context.Request.Form["txtCode"];
            string send_save = SPRequest.GetFormString("sendSave");
            string user_name = Utils.ToHtml(SPRequest.GetFormString("txtUserName"));
            string title = Utils.ToHtml(SPRequest.GetFormString("txtTitle"));
            string content = Utils.ToHtml(SPRequest.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = SPRequest.GetFormInt("txtAmount");
            string password = SPRequest.GetFormString("txtPassword");
            if (model.amount < 1)
            {
            ...
            //扣除金额
            int amountNewId = new BLL.user_amount_log().Add(model.id, model.user_name, SPEnums.AmountTypeEnum.Convert.ToString(), amout * -1, "用户兑换积分", 1);
            //增加积分
            if (amountNewId < 1)
            {
            ...
            {
                //返还金额
                new BLL.user_amount_log().Add(model.id, model.user_name, SPEnums.AmountTypeEnum.Convert.ToString(), amout, "用户兑换积分失败，返还金额", 1);
                context.Response.Write("{\"status\":0, \"msg\":\"转换过程中发生错误，请重新提交！\"}");
                return;
            }
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            decimal amount = SPRequest.GetFormDecimal("order_amount", 0);
            int payment_id = SPRequest.GetFormInt("payment_id");
            if (amount == 0)
            {
            ...
            new BLL.user_amount_log().Add(model.id, model.user_name, SPEnums.AmountTypeEnum.Recharge.ToString(), order_no,
                payment_id, amount, "账户充值(" + new BLL.payment().GetModel(payment_id).title + ")", 0);
            //保存成功后返回订单号
            context.Response.Write("{\"status\":1, \"msg\":\"订单保存成功！\", \"url\":\"" + new Web.UI.BasePage().linkurl("payment", "confirm", SPEnums.AmountTypeEnum.Recharge.ToString().ToLower(), order_no) + "\"}");
            return;
        ...
        {
            string goods_id = SPRequest.GetFormString("goods_id");
            int goods_quantity = SPRequest.GetFormInt("goods_quantity", 1);
            if (goods_id == "")
            {
        ...
        {
            string goods_id = SPRequest.GetFormString("goods_id");
            int goods_quantity = SPRequest.GetFormInt("goods_quantity");
            if (goods_id == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品参数有误！\"}");
                return;
        ...
        private void order_save(HttpContext context)
        {
            //获得传参信息
            int payment_id = SPRequest.GetFormInt("payment_id");
            int express_id = SPRequest.GetFormInt("express_id");
            string accept_name = Utils.ToHtml(SPRequest.GetFormString("accept_name"));
            string post_code = Utils.ToHtml(SPRequest.GetFormString("post_code"));
            string telphone = Utils.ToHtml(SPRequest.GetFormString("telphone"));
            string mobile = Utils.ToHtml(SPRequest.GetFormString("mobile"));
            string address = Utils.ToHtml(SPRequest.GetFormString("address"));
            string message = Utils.ToHtml(SPRequest.GetFormString("message"));
            //获取订单配置信息
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = SPcms.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = SPcms.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = SPRequest.GetQueryString("order_no");
        ...
        {
            int article_id = SPRequest.GetInt("id", 0);
            int click = SPRequest.GetInt("click", 0);
            int count = 0;
            if (article_id > 0)
            {
        ...
        {
            int attach_id = SPRequest.GetInt("id", 0);
            int count = 0;
            if (attach_id > 0)
            {
        ...
        private string verify_code(HttpContext context, string strcode)
        {
            ...
            if (strcode.ToLower() != (context.Session[SPKeys.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[SPKeys.SESSION_CODE] = null;
            return "success";
        }
        //校检网站验证码
        private string verify_sms_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[SPKeys.SESSION_SMS_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[SPKeys.SESSION_SMS_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[SPKeys.SESSION_SMS_CODE] = null;
            return "success";
        }
        #endregion END通用方法=================================================
