...
namespace SPcms.Web.tools
{
    ...
    public class download : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            int id = SPRequest.GetQueryInt("id");
            //获得下载ID
            if (id < 1)
            {
            ...
            if (model.point > 0)
            {
                ...
                //防止重复扣积分
                string cookie = Utils.GetCookie(SPKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString());
                if (cookie != model.id.ToString())
                {
                    ...
                    //写入Cookie
                    Utils.WriteCookie(SPKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString(), model.id.ToString(), 8640);
                }
            }
            //下载次数+1
