...
namespace SPcms.Web.Plugin.Advert.admin
{
    public partial class bar_list : SPcms.Web.UI.ManagePage
    {
        ...
        {
            this.aid = SPRequest.GetQueryInt("aid");
            this.keywords = SPRequest.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("plugin_advert_bar", SPEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定广告位
        ...
        {
            this.page = SPRequest.GetQueryInt("page", 1);
        ...
        {
            ChkAdminLevel("plugin_advert_bar", SPEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.advert_banner bll = new BLL.advert_banner();
            ...
            AddAdminLog(SPEnums.ActionEnum.Edit.ToString(), "修改广告内容排序"); //记录日志
        ...
        {
            ChkAdminLevel("plugin_advert_bar", SPEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.advert_banner bll = new BLL.advert_banner();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(SPEnums.ActionEnum.Delete.ToString(), "删除广告内容成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
