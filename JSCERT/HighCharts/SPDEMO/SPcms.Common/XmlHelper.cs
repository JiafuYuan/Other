...
namespace SPcms.Common
{
    public class XmlHelper
    {
        ...
        public static XmlNode selectsingleNodes(string filePath, string xPath)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
                ...
                if (xn == null)
                {
                  XmlElement xx=  doc.CreateElement(xPath);

                  
                    XmlNode xn1 = doc.SelectSingleNode("menu");
                    xn1.AppendChild(xx);
                    doc.Save(filePath);
                    xn = doc.SelectSingleNode(@"menu/"+xPath);
                }
        ...
        public static XmlElement  creatnode(string filePath, string xPath)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
        ...
        /// <returns></returns>
        public static void delALLnode(string filePath, string xPath)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
        ...
        public static void savenode(string filePath, string xPath, XmlNode xn)
        {
            XmlDocument doc = new XmlDocument();
            doc.Load(filePath);
        ...
        public static bool isexit(string filePath, string site)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
                doc.Load(filePath);
                XmlNode xn1 = doc.SelectSingleNode(@"menu/"+site);
                if (xn1 == null)
                {
                    return false;
                }
