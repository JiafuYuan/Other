...
namespace SPcms.DAL
{
    ...
    public partial class channel
    {
        ...
        /// <returns></returns>
        public bool updatenewfiled(Model.channel model)
        {
            Model.channel oldModel = GetModel(model.id); //旧的数据
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
        ...
        public DataSet GetListview(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" * ");
            strSql.Append(" FROM View_channel ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public void FieldAdd(string channelid,string filedid)
        {
            StringBuilder strSql2 = new StringBuilder();
            strSql2 = new StringBuilder();
            strSql2.Append("insert into " + databaseprefix + "channel_field(");
            ...
            SqlParameter[] parameters2 = {
					                    new SqlParameter("@channel_id", SqlDbType.Int,4),
					                    new SqlParameter("@field_id", SqlDbType.Int,4)};
            parameters2[0].Value = channelid;
            parameters2[1].Value = filedid;
        ...
        public DataTable getdtbychannelid(string  cid)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  view_channel");
            strSql.Append(" where build_path='" + cid + "'");
