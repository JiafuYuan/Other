...
namespace SPcms.DAL
{
    ...
    public partial class article
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet GetImgUrl(string tablename, string fieldname)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from " + tablename + " where " + fieldname + " <> ''");
            return DbHelperSQL.Query(strSql.ToString());
        }

        /// <summary>
        /// 获取 指定表指定值 中存在图片的content
        /// </summary>
        /// <returns></returns>
        public DataSet GetImgByContent()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from dt_article where content like '%<img src=%'");
            return DbHelperSQL.Query(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  max(id) FROM " + databaseprefix + "article");
