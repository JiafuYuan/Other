...
namespace SPcms.DAL
{
    ...
    public partial class navigation
    {
        ...
        /// <summary>
        /// 查询是否存在该记录根据频道id,单页id
        /// </summary>
        public bool Exists(string name,string cid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from " + databaseprefix + "navigation");
            strSql.Append(" where channel_id=@channel_id ");
            SqlParameter[] parameters = {
					new SqlParameter("@channel_id",SqlDbType.VarChar,10)};
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.navigation GetModel(string nav_name,string id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,nav_type,name,title,sub_title,link_url,sort_id,is_lock,remark,parent_id,class_list,class_layer,channel_id,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "navigation ");
            strSql.Append(" where channel_id=@channel_id");
            SqlParameter[] parameters = {
					new SqlParameter("@channel_id", SqlDbType.NVarChar,50)};
            parameters[0].Value = id;
