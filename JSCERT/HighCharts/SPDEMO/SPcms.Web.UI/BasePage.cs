...
namespace SPcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        {    
            //如果Session为Null
            if (HttpContext.Current.Session[SPKeys.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie(SPKeys.COOKIE_USER_NAME_REMEMBER, "SPcms");
                string password = Utils.GetCookie(SPKeys.COOKIE_USER_PWD_REMEMBER, "SPcms");
                if (username != "" && password != "")
                {
                    ...
                    if (model != null)
                    {
                        HttpContext.Current.Session[SPKeys.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        public Model.users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Model.users model = HttpContext.Current.Session[SPKeys.SESSION_USER_INFO] as Model.users;
                if (model != null)
                {
        ...
        private string GetFirstPath()
        {
            ...
            if (requestPath.StartsWith(config.webpath + SPKeys.DIRECTORY_REWRITE_ASPX + "/"))
            {
                indexNum = (config.webpath + SPKeys.DIRECTORY_REWRITE_ASPX + "/").Length;
            }
