...
namespace SPcms.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "get_builder_html": //生成静态页面
                    ...
                    break;
                case "get_baidu_map":
                    get_baidu_map(context);
                    break;
            }

        }

        private void get_baidu_map(HttpContext context)
        {
            Model.siteconfig siteConfig = new BLL.siteconfig().loadConfig();
             context.Response.Write(siteConfig.webcompany + "|" + siteConfig.webaddress);
        }

        #region 验证扩展字段是否重复============================
        private void attribute_field_validate(HttpContext context)
        {
            string column_name = SPRequest.GetString("param");
        ...
        {
            string build_path = SPRequest.GetString("param");
            string old_build_path = SPRequest.GetString("old_build_path");
        ...
        {
            string channel_name = SPRequest.GetString("param");
            string old_channel_name = SPRequest.GetString("old_channel_name");
        ...
        {
            string new_name = SPRequest.GetString("param");
            string old_name = SPRequest.GetString("old_name");
        ...
        {
            string navname = SPRequest.GetString("param");
            string old_name = SPRequest.GetString("old_name");
        ...
        {
            string user_name = SPRequest.GetString("param");
        ...
        {
            string filePath = SPRequest.GetFormString("remotepath");
        ...
        private void get_navigation_list(HttpContext context)
        {
            ...
            DataTable dt = new BLL.navigation().GetDataList(0, SPEnums.NavigationEnum.System.ToString());
        ...
        private void get_navigation_childs(HttpContext context, DataTable oldData, int parent_id, string parent_name, int role_type,List<Model.manager_role_value> ls)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                //输出开始标记
                if (first && parent_id > 0 && parent_name != "sys_contents")
                {
                    first = false;
                    isWrite = true;
                    context.Response.Write("<ul>\n");
        ...
        private void edit_order_status(HttpContext context)
        {
            ...
            string order_no = SPRequest.GetString("order_no");
            string edit_type = SPRequest.GetString("edit_type");
            if (order_no == "")
            {
            ...
            switch (edit_type.ToLower())
            {
                case "order_confirm": //确认订单
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", SPEnums.ActionEnum.Confirm.ToString()))
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Confirm.ToString(), "确认订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                ...
                case "order_payment": //确认付款
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Confirm.ToString(), "确认付款订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                ...
                case "order_express": //确认发货
                    ...
                    string express_no = SPRequest.GetFormString("express_no");
                    if (express_id == 0)
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Confirm.ToString(), "确认发货订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                ...
                case "order_complete": //完成订单=========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Confirm.ToString(), "确认交易完成订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                ...
                case "order_cancel": //取消订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", SPEnums.ActionEnum.Cancel.ToString()))
                    {
                    ...
                    int check_revert1 = SPRequest.GetFormInt("check_revert");
                    if (check_revert1 == 1)
                    {
                        //如果存在积分换购则返还会员积分
                        if (model.user_id > 0 && model.point < 0)
                        ...
                        {
                            new BLL.user_amount_log().Add(model.user_id, model.user_name, SPEnums.AmountTypeEnum.BuyGoods.ToString(), model.order_amount, "取消订单退还金额，订单号：" + model.order_no);
                        }
                    }
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Cancel.ToString(), "取消订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"取消订单成功！\"}");
                    break;
                case "order_invalid": //作废订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", SPEnums.ActionEnum.Invalid.ToString()))
                    {
                    ...
                    int check_revert2 = SPRequest.GetFormInt("check_revert");
                    if (check_revert2 == 1)
                    {
                        //扣除购物赠送的积分
                        if (model.user_id > 0 && model.point > 0)
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Invalid.ToString(), "作废订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"作废订单成功！\"}");
                    break;
                case "edit_accept_info": //修改收货信息====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", SPEnums.ActionEnum.Edit.ToString()))
                    {
                    ...
                    string accept_name = SPRequest.GetFormString("accept_name");
                    string province = SPRequest.GetFormString("province");
                    string city = SPRequest.GetFormString("city");
                    string area = SPRequest.GetFormString("area");
                    string address = SPRequest.GetFormString("address");
                    string post_code = SPRequest.GetFormString("post_code");
                    string mobile = SPRequest.GetFormString("mobile");
                    string telphone = SPRequest.GetFormString("telphone");
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Edit.ToString(), "修改收货信息，订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"修改收货人信息成功！\"}");
                    break;
                case "edit_order_remark": //修改订单备注=================================
                    string remark = SPRequest.GetFormString("remark");
                    if (remark == "")
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Edit.ToString(), "修改订单备注，订单号:" + model.order_no); //记录日志
                ...
                case "edit_real_amount": //修改商品总金额================================
                    ...
                    decimal real_amount = SPRequest.GetFormDecimal("real_amount", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Edit.ToString(), "修改商品金额，订单号:" + model.order_no); //记录日志
                ...
                case "edit_express_fee": //修改配送费用==================================
                    ...
                    decimal express_fee = SPRequest.GetFormDecimal("express_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Edit.ToString(), "修改配送费用，订单号:" + model.order_no); //记录日志
                ...
                case "edit_payment_fee": //修改支付手续费=================================
                    ...
                    decimal payment_fee = SPRequest.GetFormDecimal("payment_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, SPEnums.ActionEnum.Edit.ToString(), "修改支付手续费，订单号:" + model.order_no); //记录日志
        ...
        private void sms_message_post(HttpContext context)
        {
            ...
            string mobiles = SPRequest.GetFormString("mobiles");
            string content = SPRequest.GetFormString("content");
            if (mobiles == "")
            {
        ...
        private int GetIsLoginAndIsStaticstatus()
        {
            ...
            if (adminInfo == null)
                return  -1;
            else if (!new BLL.manager_role().Exists(adminInfo.role_id, "app_builder_html", SPEnums.ActionEnum.Build.ToString()))
                return -2;
