...
namespace SPcms.Web.Netadmin.weixin
{
    public partial class weixin_mes_list : Web.UI.ManagePage
    {
        ...
        {
            this.type_id = SPRequest.GetQueryInt("type_id");
            this.keywords = SPRequest.GetQueryString("keywords");

            this.pageSize = GetPageSize(10); //每页数量
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_message", SPEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("user_message", SPEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_message bll = new BLL.user_message();
            ...
            AddAdminLog(SPEnums.ActionEnum.Delete.ToString(), "删除站内短消息成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
