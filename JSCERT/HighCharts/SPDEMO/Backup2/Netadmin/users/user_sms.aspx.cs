...
namespace SPcms.Web.admin.users
{
    public partial class user_sms : Web.UI.ManagePage
    {
        ...
        {
            mobiles = SPRequest.GetString("mobiles");
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_sms", SPEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo(mobiles);
        ...
        {
            ChkAdminLevel("user_sms", SPEnums.ActionEnum.Add.ToString()); //检查权限
            //检查短信内容
            ...
            if (rblSmsType.SelectedValue == "1")
            {
                ...
                if (result)
                {
                    AddAdminLog(SPEnums.ActionEnum.Add.ToString(), "发送手机短信"); //记录日志
